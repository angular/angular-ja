...
別の方法として、そこに`<router-outlet>`を指定することで、トップコンポーネントをルーティングホストにしてください。
...
遅延ロードするモジュールによってもう一度そのモジュールをインポートするすることは、[誤ったふるまい](guide/ngmodule-faq#q-why-bad)を引き起こしかねず、
それはみつけて診断することが難しいでしょう。

この問題を防ぐために、ルートアプリケーションインジェクターからモジュールまたはサービスを注入しようと試みるコンストラクターを書きましょう。
