...
`HttpClient` がfetchバックエンドを使用する場合、レスポンスには、そのレスポンスがリダイレクトの結果であるかどうかを示す `redirected` プロパティが含まれます。このプロパティはネイティブのFetch API仕様に沿っており、リダイレクトシナリオを処理するインターセプターで有用です。
