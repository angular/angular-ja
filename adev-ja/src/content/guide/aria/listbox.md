...
`value`モデルシグナルは、選択されたアイテムへの双方向バインディングを提供します。`selectionMode="explicit"`では、ユーザーはSpaceキーまたはEnterキーを押してオプションを選択します。リストボックスとコンボボックス、オーバーレイ配置を組み合わせたドロップダウンパターンについては、[Select](guide/aria/select)パターンを参照してください。
