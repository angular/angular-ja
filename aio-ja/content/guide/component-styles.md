...
セレクター、ルール、そしてメディアクエリーついての全てを Angularアプリケーションに直接
適用できる事を意味します。
...
コンポーネントの *内部* の全ての `<h2>` 要素のスタイルに `background-color` を適用します。
...
つぎの例は、全ての `<h3>` 要素、つまり、ホストの要素から、このコンポーネントを下がり降り、
DOM の全てのそれの子の要素まで、を対象とします。
...
`styleUrls` プロパティを `@Component` デコレータのコンポーネントに追加すると、
...
`@Component.styles` 配列に追加されたスタイル文字列は、_CSS に記述されなければならない_ 、何故ならば、CLI は、インラインスタイルのプリプロセッサに適用できないからです。
...
*プリコンポーネント* 基礎の上で、どの様にカプセル化が起きるのかを制御し、
...
* 先天的なカプセル化内の Shadow DOM ホストの一つの要素は、
