...
アプリケーションの国際化は多面的な開発分野であり、アプリケーションを利用可能にし、世界中のユーザーにとって使いやすいものにすることに重点が置かれています。
...
<div class="alert is-helpful">
  ...
  Angular が使用するすべてのロケールデータは、Unicode コンソーシアムの <a href="http://cldr.unicode.org/" title="CLDR">Common Locale Data Repository (CLDR)</a> から抽出されます。
...
`i18n` 属性値を _meaning_ で始めて、それを `|` 文字で _description_ から分離することで
...
英語では、分数に応じて、「今すぐ」、「1分前」、
または「x分前」(xは実際の数) を表示できます。
他の言語はカーディナリティを異なって表現するかもしれません。
...
ゼロ分について話すには、`=0 {今すぐ}`を使用します。1分は `=1 {1分}` を使用してください。
比類のないカーディナリティは、`他の {{{minutes}} 分前}`を使用します。
...
コンポーネントテンプレート内の次のフォーマットメッセージは、コンポーネントの `gender` プロパティにバインドされ、次の文字列値 "m"、"f"、または "o" のいずれかを出力します。
...
<div class="alert is-helpful">
  ...
  ローカライズと国際化は<a href="https://en.wikipedia.org/wiki/Internationalization_and_localization">異なるが密接に関連した用語です</a>。
...
複数の言語でアプリケーションを生成するためのスクリプトの作成方法、およびさまざまなサブディレクトリからそれらを提供するように Apache2 を設定する方法の詳細については、[Philippe Martin によるこのチュートリアル](https://medium.com/@feloy/deploying-an-i18n-angular-app-with-angular-cli-fc788f17e358#.1xq4iy6fp)を読んでください。
