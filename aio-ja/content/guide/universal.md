...
[最終的なサンプルコードのダウンロード](generated/zips/universal/universal.zip)(これは[node/expressフレームワーク](https://expressjs.com/)で動作するサーバーです)
...
実際には、ランディングページの静的バージョンを配信し、ユーザーの注意を引きつけます。同時に[以下に説明する方法](#transition)で、バックグラウンドに完全なAngularアプリケーションを読み込みます。ユーザーはこのランディングページがら瞬間的なレスポンスを認識し、アプリケーションが完全に読み込まれた後、インタラクティブな体験を得ることができます。
...
あるいは、`Location`や`Document`など必要なオブジェクトに対して注入可能なAngular abstractionを探します。あなたが使用している特定のAPIを適切に置き換えることができるかもしれません。
...
このチュートリアルのデモおよび開発ツールである"in-memorywebapi"モジュールは、すべてのHTTPリクエストに割り込み、リモートデータサーバーの動作をシミュレートします。実際にはそのモジュールを削除し、サーバーのweb APIミドルウェアをここに登録します。
...
Universalアプリケーションは、何らかのWebpack拡張設定を必要とせず、Angular CLIがそういった設定を代行します。しかし、サーバーはtypescriptアプリケーションであるため、Webpackを使用してそれをトランスパイルします。
...
Angular CLIは、Universalアプリケーションを2つの異なるフォルダー、`browser`と`server`にコンパイルして包括します。
