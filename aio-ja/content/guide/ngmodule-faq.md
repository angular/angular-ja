...
Angularはその名前について認識しませんが、Angular開発者はできます。あなたがサービスプロバイダーを設定できる似たようなモジュールを書くときにこの規約にしたがってください。
...
この問題を防ぐために、ルートアプリケーションインジェクターからモジュールまたはサービスを注入するコンストラクターを書きましょう。
...
モジュールを部分を公開するために個々のファイルで`export`文を書きます。
