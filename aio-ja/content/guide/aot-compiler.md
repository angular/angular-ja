...
* *テンプレートエラーを早期に検出する*
   AOT コンパイラは、ユーザーの目に入り得る前にビルドステップ中にテンプレートバインディングエラーを検出して
   報告します。
...
Angularコンパイラのもっとも役立つ特徴の1つは、テンプレート内の式を型チェックしてそれらが実行時にクラッシュを引き起こす前にエラーを補足する能力です。
...
[AngularのIvy](guide/ivy)においては、テンプレート型チェッカーはより厳格により有能に完全に書き換えられます。つまり、以前の型チェッカーが検出しないさまざまな新しいエラーを補足できることを意味します。

結果として、以前にビューエンジンのもとでコンパイルされたテンプレートは、Ivyでは型チェックに失敗する可能性があります。これが起こり得るのは、Ivyのより厳格なチェックが真のエラーを補足するため、またはアプリケーションのコードが正しく型を付けられていないため、またはアプリケーションが誤りのあるもしくは十分に明確でない型付けをしているライブラリを使用しているためです。
