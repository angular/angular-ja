...
Angular CLIを使用して、`Hero`という名前の新しいクラスを生成します。
...
TypeScriptコンパイラは、`public`コンストラクタパラメータごとにパブリックフィールドを生成し、
ヒーローを作成するときにそのフィールドにパラメータの値を自動的に割り当てます。

`alterEgo`はオプションであるため、コンストラクタで省略することができます。 `alterEgo?`の疑問符（?）に注目してください。
...
Angular CLIを使用して、`HeroForm`という名前の新しいコンポーネントを生成します。
...
* "hero-form"という`@Component`のセレクタ値は、このフォームを`<hero-form>`タグによって親テンプレートに配置できることを意味します。
...
<div class="l-sub-section">
  ...
  1. `@NgModule`デコレータに定義されている`imports`リストに`FormsModule`を追加します。これにより、
...
このコードは、能力のリストのそれぞれの能力ごとに`<option>`タグを繰り返します。
...
<div class="l-sub-section">
  ...
  それぞれの`FormControl`は、`name`属性に割り当てた名前で登録されます。
...
このフォームは改善できます。_Name_の入力ボックスは必須項目で、それをクリアすると、バーが赤色に点灯しています。
...
このユーザーエクスペリエンスは開発者の選択です。開発者によっては、メッセージを常に表示することを望みます。
...
* `@Component`デコレータをもつフォームコンポーネントクラス。
