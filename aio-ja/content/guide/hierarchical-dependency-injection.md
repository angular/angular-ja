...
このガイドでは、このシステムとそれをどのように使用するかを探索します。
...
サービス自体の(通常はアプリのルートレベルで)`@Injectable()`デコレーターでプロバイダーを指定すると、CLIのプロダクションビルドで使用されている最適化ツールは*ツリーシェイキング*を実行できます。これによって、アプリで使用されていないサービスが削除されます。ツリーシェイキングの結果、バンドルサイズがより小さくなります。

* [ツリーシェイク可能なプロバイダー](guide/dependency-injection-providers#tree-shakable-providers)について詳しく学んでください。

あなたはアプリのいろいろな場所で`UserService`を注入する可能性があり、毎回同じサービスインスタンスを注入したいと思うでしょう。`root`インジェクターを通して`UserService`を提供することはよい選択です。そして、アプリでサービスを生成するときに[Angular CLI](cli)を使用するとデフォルトで設定されます。
...
*NgModuleレベル*のプロバイダーは`@NgModule()`の`providers`メタデータオプション、あるいは`@Injectable()`の`providedIn`オプション(ルートの`AppModule`以外のモジュール)で指定できます。
...
`HeroService`をコンポーネントレベル(`HeroComponent`のメタデータ内)で提供してください。
...
サービスクラスの`providedIn`メタデータオプションでは、デコレーターが付与されたクラスをサービスのプロバイダーとして使用するためのインジェクターを指定します(通常は`root`)。
...
そのインスタンスをアプリケーション全体で利用できるようにする責務をもつことを教えます。
...
非ルートのNgModuleのために`providedIn`メタデータオプションを使って、プロバイダーのスコープをそのモジュールに限定するため、モジュールレベルでプロバイダーを設定することができます。
これは`@Injectable()`メタデータでルート以外のモジュールを指定するのと同じですが、この方法で提供されるサービスはツリーシェイクできないというのが異なる点です。
...
これを実行して、複数のアプリで共有されるサービスの非デフォルトのプロバイダーを設定できます。
...
このガイドのツアー・オブ・ヒーローズアプリケーションに関する変更について考えてみましょう。
一番上にあるのは`HeroesListComponent`のようないくつかのサブコンポーネントをもつ`AppComponent`です。
`HeroesListComponent`は`HeroTaxReturnComponent`を複数インスタンス保持し管理します。
次の図は3つの`HeroTaxReturnComponent`インスタンスが同時に開いているときの、この3階層のコンポーネントツリーの状態を表しています。
...
プロバイダーをトップレベル(通常はルートの`AppModule`)のルートインジェクターに登録するだけの場合、インジェクターのツリーはフラットになります。
...
セッターはコンポーネント自身の`HeroTaxReturnService`インスタンスに入力される申告書によって初期化します。
...
それから、子コンポーネント(B)を作成します。
それは、コンポーネント(B)に適用した特殊な機能をもつ、`CarService`と`EngineService`の_特殊化した_プロバイダーを自身に定義しています。
