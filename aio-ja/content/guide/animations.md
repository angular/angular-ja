...
* モーションはユーザ体験を大幅に向上させます。アニメーションはユーザーの操作に対するアプリケーションの応答を検出する機会を与えます。
...
このガイドでは、以下のセクションで説明されているような基本的なAngularアプリの作成に精通していることを前提としています:
...
最初の部分、`duration`は必須です。デュレーションは、クォートのない純粋な数値で表現されたミリ秒、またはクォートと時間指定子を含む秒単位で表現することができます。例えば、10分の1秒のデュレーションは、次のように表現することができます:
...
3番目の引数、`easing`は実行時にアニメーションの[加速と減速](http://easings.net/)を制御します。例えば、`ease-in`は、アニメーションをゆっくりと開始させ、進行とともに速度を上げます。
...
`trigger()`関数は、変更を監視するためにプロパティ名を記述します。変更が発生すると、トリガーはその定義に含まれるアクションを開始します。これらのアクションは、後で説明するような、遷移関数やその他の関数にすることができます。
...
Angularアニメーションの高度な機能については、アニメーションセクション以下の[遷移とトリガー](guide/transition-and-triggers)以降で学ぶことができます。
