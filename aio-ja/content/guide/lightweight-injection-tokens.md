...
Angular スタイルガイドでは、"Component" サフィックスを使用してコンポーネントに名前を付けることを提案しています。
"LibHeaderComponent" の例は、この規則に従います。

コンポーネントとそのトークンの関係を維持しながらそれらを区別するには、コンポーネントのベース名にサフィックス "`Token`" を付けて、軽量インジェクショントークン "`LibHeaderToken`" に名前を付けるスタイルをお勧めします。
