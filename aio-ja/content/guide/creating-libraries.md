...
F毎回異なる渡された値を含むドロップダウンのようなコンポーネントの場合は、それを共有ライブラリのコンポーネントとして指定できます。
...
* `ng add` がライブラリをプロジェクトに追加できるようにインストール schematic を含めてください。

* `ng generate` がプロジェクト内の定義済みの成果物 (コンポーネント、サービス、テストなど) を足場にできるように、生成スキーマをライブラリに含めます。

* `ng update` がライブラリの依存関係を更新し、新しいリリースでの破壊的変更の移行を提供できるように、更新 schematic を含めてください。

詳細については、[Schematics — 概要](https://blog.angular.io/schematics-an-introduction-dc1dfbc2a2b2)を参照してください。

## ライブラリを公開する

Angular CLI と npm パッケージマネージャを使用して、ライブラリを npm パッケージとして構築および公開します。
...
## 仲間の依存関係に TypeScript パスマッピングを使用する

Angular ライブラリはすべての `@angular/*` 依存関係を仲間の依存関係として一覧にするべきです。
...
ライブラリを開発している間、ライブラリが正しくコンパイルされることを保証するために `devDependencies` を通してすべての仲間の依存関係をインストールしなければなりません。
...
ライブラリが TypeScript 設定ファイル `./tsconfig.json` の中で使用しているすべての仲間の依存関係をリストアップし、それらをアプリケーションの `node_modules` フォルダの中のローカルコピーを指すようにしてください。
...
自分のアプリケーションでライブラリを使用するためにライブラリを npm パッケージマネージャに公開する必要はありませんが、最初にビルドする必要があります。
