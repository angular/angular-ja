...
E2E は、予想とおりに、全ての子をインスタンス化と表示化されることをテストします：
...
ここで `NameParentComponent` は、すべて空白の名前を含む名前のバリエーションをデモンストレーションします：
...
子の `EventEmitter` プロパティは、ひとつの ***出力プロパティ*** です、
...
データ・バインド(データの結び付き)をできません。

子のコンポーネントに相当する　`<countdown-timer>` タグ内に `#timer` ローカル変数を設置することは出来ます。
...
カウントダウン・タイマーが *Stop* ボタンのクリックによって一時停止することをテストしましょう：
...
*ローカル変数* は、簡素で簡単です。しかし、制限があります、なぜならば
親-子の結び付きは完全に親のテンプレート内で無ければなりません。
親コンポーネントは *それ自身* に子へのアクセスは何ひとつ無いです。

仮に親のコンポーネント *クラス* のインスタンスが、子のコンポーネントの変数の読み書きの必要性、または子の
コンポーネント・メソッド呼び出しの必要性があったとしても、*ローカル変数* テクニックを使うことはできません、
...
`MissionControlComponent` には、このガードを加えられない、なぜならば、親として、
