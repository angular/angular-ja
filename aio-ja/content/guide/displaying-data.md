...
テンプレートは、ECMAScript 2015バッククォート(<code>\`</code>)内の複数行の文字列です。
一重引用符(`'`)と同じ文字ではないバッククォート(<code>\`</code>)&mdash;は複数行にわたって
...
`template` プロパティを使って *インライン* で定義したり、
テンプレートを別のHTMLファイルで定義したり、コンポーネントのメタデータで
`@Component` デコレータの `templateUrl` プロパティを使ってリンクすることができます。
...
詳細については、[テンプレート文法](guide/template-syntax#ngFor) ページを参照してください.
...
これはテンプレート入力変数の例です。
さらに詳しいテンプレート入力変数につては、[テンプレート文法](guide/template-syntax) ページの
[マイクロ文法](guide/template-syntax#microsyntax) セクションを読んでください。
...
コンストラクタと2つのプロパティ、 `id` と `name` を持つクラスを定義しました。
...
さらに詳しい `ngIf` と `*` については、[テンプレート文法](guide/template-syntax) ページの
...
コンポーネントのヒーローのリストに3つ以上のアイテムがある場合、Angularは段落をDOMに追加し、
メッセージが表示されます。 3つ以下のアイテムがある場合、 Angularは段落を省略するので、
メッセージは表示されません。 詳細については、
[Tテンプレート文法](guide/template-syntax) ページの
...
パフォーマンスが向上し、
特に大規模なプロジェクトでは、条件付きで多数のデータバインディングを含む大量のHTMLを含めたり除外したりします。
</div>



やってみて。 配列には4つの項目があるため、メッセージが表示されます。
...
* **ngFor** はアイテムの配列を表示します。
* コンポーネントの **モデルデータ** を作成し、そのモデルのプロパティを表示するTypeScriptクラス。
* **ngIf** はブール式に基づいて条件付きでHTMLの塊を表示します。
