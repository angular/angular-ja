...
**Do** 慣例的なサフィックス付きのシンボル名（たとえば、 `Component`、
...
**Do** ファイル名に慣例的なサフィックス（たとえば、 `.component.ts`、` .directive.ts`、
...
**Do** サービスクラスはサフィックスに`Service`を付けてください。
...
**Why?** `Logger`のような明確なサービス名はサフィックスを必要としません。
...
**Why?** `Credit` などのサービス名は名詞であり、サフィックスを必要とするため、サービスであれ他のものであれ、明示的でない場合はサフィックスを付ける必要があります。
...
**Do** コンポーネントセレクターにカスタムプレフィックスを使用します。
たとえば、`toh`というプレフィックスは**T**our **o**f **H**eroesを表していて、`admin`というプレフィックスは管理者用の機能領域を表します。
...
**Do** E2Eのテスト仕様ファイルは、機能の後に`.e2e-spec`というサフィックスを付けます。
...
**Do** _RoutingModule_クラス名に`RoutingModule`サフィックスを付けます。
...
**Do** プレフィックス `./` を付けて、_コンポーネント相対_ URLを指定します。
...
**Why?** `./`プレフィックスは相対URLの標準的な構文です。そのプレフィックスなしでのAngularの機能に依存しないでください。
...
### _アウトプット_ プロパティにプレフィックスをつけてはいけません
...
**Do** イベントには `on` プレフィックスなしの名前を付けます。
...
**Do** イベントハンドラーメソッドには `on` プレフィックスにイベント名を続けて名前を付けます。
...
**Why?** Angularでは、`on-*` という[代替構文](guide/binding-syntax)が使えます。イベント自体に`on`プレフィックスが付いていると、`on-onEvent`バインディング式になってしまいます。
