...
詳細については、[Angular Ahead-of-time WebPack Plugin のドキュメント](https://github.com/angular/angular-cli/tree/master/packages/ngtools/webpack)を参照してください。
...
AOT コンパイラに変換設定を使用するように指示するには、`angular.json` CLI設定ファイルで 3 つの "i18n" ビルド設定オプションを設定します。
...
`angular.json` Angular CLI設定ファイルの `configurations` セクションで警告レベルを指定します。次の例は、警告レベルを error に設定する方法を示しています。
