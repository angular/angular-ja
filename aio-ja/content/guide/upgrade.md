...
AngularJS のソースコードを書き、構成 **しない** かについて豊富な情報が含まれています。
...
これは AngularJS の `$location` プロバイダーの代わりを登録します。一度登録されたら、遷移時に実行される AngularJS における全ての遷移やルーティングのブロードキャストメッセージ、いずれかの必要なダイジェストサイクルは Angular によって制御されます。これにより、１つのやり方で一貫してハイブリッドアプリケーションの両側で遷移を行うことができます。
...
ともかく、サービスをそれを使っているコンポーネントの移行なしで
Angular に移行することを達成しました。
...
Angular ではアプリケーションのコンポーネント木の先頭にある、
...
テストだけはアップグレードの家庭でそのままにしておくことはできません。しかし、
