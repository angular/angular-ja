...
[属性バインディング](guide/templates/binding#binding-dynamic-properties-and-attributes)テンプレート構文を使用して、アクセシビリティ関連属性の値を制御します。
...
再利用可能なWAI-ARIAインタラクションパターンを必要とするカスタムスタイルのコンポーネントには、[Angular Aria](guide/aria/overview)がアコーディオン、コンボボックス、リストボックス、メニュー、タブ、ツールバーなどのパターン向けのヘッドレスディレクティブを提供します。
これらのディレクティブは、キーボード操作、ARIA属性、フォーカス管理、スクリーンリーダーサポートを処理する一方で、アプリケーションのHTML構造とスタイリングは開発者が提供できるようにします。
