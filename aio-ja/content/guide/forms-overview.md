...
真のソースが与えられた時点でのform要素の値と状態を提供します。リアクティブフォームでは、フォームモデルが真のソースになります。上記の例では、フォームモデルは`FormControl`インスタンスです。

<figure>
  <img src="generated/images/guide/forms-overview/key-diff-reactive-forms.png" alt="Reactive forms key differences">
</figure>

リアクティブフォームでは、フォームモデルはコンポーネントクラスで明示的に定義されています。次に、リアクティブフォームディレクティブ(この場合は`FormControlDirective`)は、バリューアクセサ(`ControlValueAccessor`インスタンス)を使って既存の`FormControl`インスタンスをビュー内の特定のform要素にリンクします。
...
1. input要素にアタッチされているコントロールバリューアクセサは`FormControl`インスタンスの`setValue()`メソッドをトリガーします。
1. `FormControl`インスタンスは`valueChanges`オブジェクトを通して新しい値を発行します。
1. `valueChanges`Observableのサブスクライバーが新しい値を受け取ります。
