...
これをコンポーネントクラスインスタンス(*コンポーネント*)と、そのユーザー向けテンプレートとの対話を通して実現します。
...
データバインディングの最初の形式&mdash;補完&mdash;から始めて、テンプレート HTML でできることの豊富さを確認しましょう。
...
補完を使用すると、計算された文字列を HTML 要素タグ間および属性割り当て内のテキストに組み込むことができます。
テンプレート式は、
これらの文字列を計算するために使用するものです。

補完の <live-example></live-example> では、
このセクションで説明されているすべての構文とコードスニペットを示しています。

### `{{...}}` を補完する

補完とは、マークアップされたテキストに式を埋め込むことです。
デフォルトでは、補完は二重中括弧 `{{` と `}}` を区切り文字として使います。

次のスニペットでは、`{{ currentCustomer }}` が補完の例です。
...
最後に、この合成補完の結果を **要素またはディレクティブのプロパティ** に割り当てます。
...
ただし、
補完は Angular がプロパティバインディングに変換する特別な構文です。

`{{` および `}}` 以外のものを使用する場合は、
`Component` メタデータの
[interpolation](api/core/Component#interpolation)
オプションを使用して補完の区切り文字を設定できます。
...
プロパティ名、またはメソッド呼び出しは標準的であるべきです、しかし、たまのブール否定、`!` はOKです。
...
[イベントバインディング](guide/template-syntax#event-binding) セクション内でテンプレート文について見ます。
`(event)="statement"`のように `=` 記号の右側の引用符で囲まれています。
...
補完以外のさまざまなデータバインディング構文について学習する準備が整いました。
...
補完以外のバインディングタイプは、等号の左側に **ターゲット名** があり、区切り(`[]`、`()`)で囲まれているか、
または接頭辞が前に付いています(`bind-`、`on-`、`bindon-`)。

ターゲット名は _プロパティ_ 名です。それは _属性_ 名のように見えるかもしれませんが、そうではありません。
違いを理解するには、テンプレート HTML について考えるための新しい方法を開発する必要があります。
...
まだ Angular テンプレート内でこの方法を使用して構造を作成し、属性の値を初期化しています。
